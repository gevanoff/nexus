...
Gateway persistence is stored on the host under `${NEXUS_RUNTIME_ROOT}/gateway/` and bind-mounted into the container. Local repo-root runs can keep `.env.example`'s `NEXUS_RUNTIME_ROOT=./.runtime`; deployed hosts should set an explicit absolute path in their selected deploy env file.
- **Read-write data** (SQLite DBs, tool logs, cached UI assets): `${NEXUS_RUNTIME_ROOT}/gateway/data/` → `/var/lib/gateway/data`
- **Read-only operator config** (model aliases, agent specs, tools registry): `${NEXUS_RUNTIME_ROOT}/gateway/config/` → `/var/lib/gateway/config`
...
Use this utility to populate the shared reference-audio pool used by Gateway and TTS containers (`${NEXUS_RUNTIME_ROOT}/tts_refs`).
...
- Persistent state and large artifacts live under `${NEXUS_RUNTIME_ROOT}/` (bind mounts), not Docker named volumes. Local repo-root runs use `.env.example`'s `./.runtime` default; deployed hosts should set an explicit absolute `NEXUS_RUNTIME_ROOT`.
- Gateway model alias config lives at `${NEXUS_RUNTIME_ROOT}/gateway/config/model_aliases.json`.
...
- **General agents**: `POST /v1/agent/run` executes an agent spec from `AGENT_SPECS_PATH` with tiered tool access. Available tools are exposed through `GET /v1/tools`, and agents can introspect their allowed tools through `tool_manifest`. Tier-1 agents and provider-neutral gateway execution can use `nexus_agent_api` to operate the scoped Agent API as the authenticated caller; personal tokens and workspace bindings are never exposed to the model.
...
- **Coding workspaces**: `/ui/coding`, `/ui/api/coding/*`, and `/v1/coding/*` create isolated git clones under `CODING_WORKSPACE_ROOT`. `/api/v1/*` adds a machine-oriented API for external agents with scoped personal tokens, workspace lifecycle and task management, constrained execution, artifact transfer, cursor pagination, consistent errors, and an authenticated OpenAPI document. Each workspace gets a branch, scoped file/tree/search/command/git APIs, optional autonomous agent runs, local checkpoint commits, push/draft-PR actions, and a workspace message channel for steering after the initial run. Long-running work also has a durable milestone plan, bounded run horizons, periodic context reconstruction from workspace state, and retained run history so a paused or interrupted job can continue without depending on one unbounded conversation.
