...
Observed from the local Nexus hosts on 2026-05-16:
...
| `ai1` | vLLM | `unsloth/Qwen3-30B-A3B-GGUF:Q4_K_M` at `max_model_len=2048`; `unsloth/Qwen3-Embedding-4B` | Fast short edits and embeddings | Live probe reported two 24 GB RTX 3090 devices; topology docs should be rechecked before pinning GPU IDs. |
| `ada2` | vLLM | `unsloth/Qwen3-30B-A3B-FP8` at `max_model_len=2048` | Strong CUDA lane, currently too short for repo coding | RTX 6000 Ada 48 GB. Media services share this GPU, so long-context coding needs an explicit resource window. |
