...
class LifecycleManager:
    ...
    def _docker_probe_command() -> str:
        return (
            ...
            "preferred_output=\"\"; fallback_output=\"\"; default_output=\"\"; "
            "for sock in "
            "\"${DOCKER_HOST#unix://}\" "
            "\"${COLIMA_HOME:-}/default/docker.sock\" "
            "\"/Volumes/ai_data/var/lib/colima/default/docker.sock\" "
            "\"/ai-data/var/lib/colima/default/docker.sock\" "
            "\"${HOME:-}/.colima/default/docker.sock\"; do "
            "if [ -n \"$sock\" ] && [ -S \"$sock\" ]; then "
            "probe_output=$(DOCKER_HOST=\"unix://$sock\" \"$docker_bin\" ps --format '{{.Names}}\\t{{.Status}}' 2>/dev/null || true); "
            "if [ -n \"$probe_output\" ]; then preferred_output=\"$probe_output\"; break; fi; "
            "if [ -z \"$fallback_output\" ]; then fallback_output=\"$probe_output\"; fi; "
            "fi; "
            "done; "
            "default_output=$(\"$docker_bin\" ps --format '{{.Names}}\\t{{.Status}}' 2>/dev/null || true); "
            "if [ -n \"$preferred_output\" ]; then printf '%s\\n' \"$preferred_output\"; "
            "elif [ -n \"$default_output\" ]; then printf '%s\\n' \"$default_output\"; "
            "elif [ -n \"$fallback_output\" ]; then printf '%s\\n' \"$fallback_output\"; "
            "fi; "
            "fi; true"
        )
