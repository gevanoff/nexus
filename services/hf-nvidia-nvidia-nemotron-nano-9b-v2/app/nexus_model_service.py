...
def _route_kind() -> str:
    route_kind = _env("NEXUS_ROUTE_KIND", "chat").lower()
...
def _service_name() -> str:
    return _env("NEXUS_SERVICE_NAME", "hf-nvidia-nvidia-nemotron-nano-9b-v2")


def _service_title() -> str:
    return _env("NEXUS_SERVICE_TITLE", "HF NVIDIA-Nemotron-Nano-9B-v2")


def _service_description() -> str:
    return _env("NEXUS_SERVICE_DESCRIPTION", "HuggingFace model adapter for nvidia/NVIDIA-Nemotron-Nano-9B-v2")
...
def root() -> dict[str, Any]:
    return {
        ...
        "route_kind": _route_kind(),
        "mode": _resolve_mode(),
        "port": _int_env("NEXUS_SERVICE_PORT", 8610),
        "endpoints": {
            "health": "/health",
            "healthz": "/healthz",
            "readyz": "/readyz",
...
def models() -> dict[str, Any]:
    return {
        "object": "list",
        "data": [{"id": _env("NEXUS_MODEL_ID", "nvidia/NVIDIA-Nemotron-Nano-9B-v2"), "object": "model", "owned_by": _env("NEXUS_MODEL_OWNER", "nexus")}],
    }
...
def metadata() -> dict[str, Any]:
    return {
        ...
        "backend_class": _env("NEXUS_SERVICE_BACKEND_CLASS", "hf_nvidia_nvidia_nemotron_nano_9b_v2"),
        "route_kind": _route_kind(),
        "mode": _resolve_mode(),
