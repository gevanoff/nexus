...
def _public_proxy_hostname(hostname: str) -> str:
    ...
    if host.lower() in {"stackrot", "ai2", "migraine", "ada2", "meltdown", "copyfail", "adada"}:
        return f"{host}.local"
    return host
...
async def _fetch_image_backend_catalog_entry(client: httpx.AsyncClient, *, backend_class: str, config: Any, checker: Any) -> Dict[str, Any]:
    ...
    if backend_class == "gpu_heavy":
        ui_url = _invokeai_ui_url() or browser_accessible_url(base) or browser_accessible_url(base)
        if ui_url:
...
async def ui_admin_model_purge(req: Request, body: ModelPurgeRequest) -> Dict[str, Any]:
    ...
    payload = await call_lifecycle_manager(
        ...
        json_body={"backend_class": backend_name, "model": model_name},
        timeout=max(600.0, lifecycle_timeout()),
    )
...
@router.post("/ui/api/admin/models/redownload", include_in_schema=False)
async def ui_admin_model_redownload(req: Request, body: ModelPurgeRequest) -> Dict[str, Any]:
    _require_admin(req)
    ...
    if not model_name:
        raise HTTPException(status_code=400, detail="model is required")
    if backend_provider_name(backend_name) != "mlx":
        raise HTTPException(status_code=400, detail="re-download is currently supported only for MLX models")
