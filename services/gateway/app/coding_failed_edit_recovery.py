...
def _successful_read_after(
    ...
    *,
    ...
    for event in events[index + 1 :]:
        ...
        if result and _normalized_path(result.get("path")) == path and "content" in result:
            return True
    return False
...
def refine_state(
    ...
    bounded read of that exact target. The existing evidence-freshness overlay
    will require a new hypothesis if that refresh postdates linked causal
    evidence.
    """
    out = dict(state)
    ...
    out["required_action"] = (
        ...
        "After that refresh, follow the evidence-freshness policy before editing again."
    )
    return out
...
def install(agent: Any, evidence_policy: Any) -> None:
    ...
    def prompt_with_failed_edit_recovery(base: Any, state: Mapping[str, Any]) -> str:
        ...
        return (
            ...
            f"elsewhere. Call coding_read_file_lines exactly once on {target} to refresh the current "
            "source for that failed edit. After the read, obey the next controller policy; newer "
            "causal evidence may require revising the structured hypothesis before editing. "
            "coding_finish remains available for a concrete blocker."
        )
