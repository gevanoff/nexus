...
@pytest.mark.asyncio
async def test_real_agent_loop_pauses_on_unchanged_semantic_acceptance_state(
    monkeypatch,
) -> None:
    task = {
        "id": "code_test",
    ...
    mission = {
        "budget_policy": {"max_no_progress_cycles": 12},
        "context_policy": {"context_reset_chars": 64_000},
        "completion_policy": {
            "require_file_changes": False,
            "require_commit_on_success": False,
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: {
            ...
            "required_action": "Change the diff or materially revise the hypothesis.",
            "summary": "Independent semantic acceptance already rejected this state.",
        },
    )
    monkeypatch.setattr(coding_agent, "_checkpoint_enabled", lambda: False)
    monkeypatch.setattr(
    ...
    assert backend_calls == 1
    assert task["agent_status"] == "paused"
    assert task["agent_stop_reason_code"] == "semantic_acceptance_state_unchanged"
    assert task["agent_runs"][-1]["status"] == "paused"
    paused = [event for event in task["agent_events"] if event.get("type") == "paused"]
    assert len(paused) == 1
    assert paused[0]["details"]["required_action"].startswith("Change the diff")
