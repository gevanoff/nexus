...
def test_long_backend_timeout_keeps_read_budget_and_divides_connect_cap_across_retries():
    ...
    assert httpx_client._connect_budget_sec(600.0) == 30.0


def test_long_backend_timeout_without_transport_retries_allows_full_connect_cap():
...
def test_short_backend_timeout_is_not_extended_or_divided_by_caps():
    timeout = httpx_client._effective_timeout(10.0, connect_retries=2)
    ...
    assert httpx_client._connect_budget_sec(10.0) is None
...
def test_blank_connect_timeout_reports_per_attempt_and_aggregate_budget_when_cap_applies():
    timeout = httpx_client._effective_timeout(600.0, connect_retries=2)
    ...
    httpx_client._ensure_request_error_text(
        ...
        timeout=timeout,
        connect_attempts=3,
        connect_budget_sec=httpx_client._connect_budget_sec(600.0),
    )

    assert str(exc) == "ConnectTimeout: connect timeout after 10s per attempt (30s budget across 3 attempts)"


def test_blank_short_connect_timeout_does_not_claim_aggregate_budget():
    ...
    httpx_client._ensure_request_error_text(
        ...
        timeout=timeout,
        connect_attempts=3,
        connect_budget_sec=httpx_client._connect_budget_sec(9.0),
    )

    assert str(exc) == "ConnectTimeout: connect timeout after 9s"
...
@pytest.mark.asyncio
async def test_instrumented_send_applies_capped_phase_timeouts_and_enriches_blank_timeout():
