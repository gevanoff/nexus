...
def _read(path: str, *, ok: bool = True, ts: float = 0) -> dict:
...
def _plan(*, ok: bool = True, ts: float = 0) -> dict:
    event = {
        "type": "tool_finished",
...
def test_compacted_plan_event_uses_durable_plan_timestamp_to_detect_staleness():
    state = freshness.refine_state(
        {
            "project_plan": {"updated_at": 100.25},
            "agent_events": [_read(TARGET, ts=101)],
        },
        _edit_state(),
    )

    assert state["action_kind"] == "evidence"
    assert state["hypothesis_freshness_source"] == "plan_timestamp"


def test_compacted_plan_event_uses_durable_plan_timestamp_to_preserve_fresh_plan():
    state = freshness.refine_state(
        {
            "project_plan": {"updated_at": 101.75},
            "agent_events": [_read(TARGET, ts=101)],
        },
        _edit_state(),
    )
...
def test_legacy_state_without_ordering_marker_is_not_invented_stale():
    state = freshness.refine_state(
