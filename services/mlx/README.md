...
- GLM-5.2 chat aliases advertise a 32768-token context, while the `coder`, `reasoning`, and `long` profiles use the runtime-verified 131072-token window. Profiled requests use their alias `max_input_tokens` budgets and the conservative Gateway token estimator. Direct or otherwise unprofiled GLM model IDs retain the legacy `MLX_GLM_MAX_INPUT_CHARS=98304` guard. Larger requests are model-capable but measured at four to five minutes of prefill/generation and can starve other clients, so compact them or use an offline workload. DeepSeek R1 remains at 65536. GLM-5.2 and DeepSeek R1 declare 1048576 and 163840 positions respectively; their latent-attention KV dimensions leave headroom on the 512GB host when only one Huge model is resident.
