...
- `meltdown` has Ubuntu 22.04, about 47GB system RAM, and a 16GB RTX 5060 Ti. It owns SDXL-Turbo, the lightweight vLLM embeddings lane, and a dedicated Qwen2.5 7B AWQ chat lane for Cinder. The chat lane is capped at an 8K context and 58% GPU-memory utilization so it can coexist with the other services. Its FlashInfer sampler is disabled because the vLLM 0.10.2 sampler stalled generation on the RTX 5060 Ti; attention still uses the automatically selected CUDA backend.
