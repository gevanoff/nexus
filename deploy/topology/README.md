...
- `ada2` keeps Nexus-owned runtime/control state under `/data/nexus-runtime`; this tree is distinct from backend-owned data roots. Backends use explicit bind-source settings for `/data/invokeai`, `/data/ltx-video`, `/data/heartmula`, `/data/followyourcanvas`, and `/data/personaplex`. The shared Hugging Face cache uses `/data/huggingface`.
