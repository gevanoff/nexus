...
Defaults are derived from env:
  ...
  - MLX_MODEL_PATH (default: mlx-community/GLM-5.2-4bit) when MLX_CONFIG_PATH is not set
...
if [[ "${#MLX_MODEL_OVERRIDES[@]}" -gt 0 ]]; then
  ...
  add_unique_model "${MLX_MODEL_PATH:-$(ns_env_get "$ENV_FILE" MLX_MODEL_PATH "mlx-community/GLM-5.2-4bit")}"
fi
