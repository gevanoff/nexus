...
SSH_DIR=""
RCLONE_REMOTE=""
LABEL="${NEXUS_GATEWAY_DB_BACKUP_LAUNCHD_LABEL:-com.nexus.gateway-db-backup}"
LAUNCHD_ROOT="${NEXUS_GATEWAY_DB_BACKUP_LAUNCHD_ROOT:-/ai-data/launchd/nexus-gateway-db-backup}"
LOG_DIR=""

usage() {
  cat <<'EOF'
Usage: deploy/scripts/install-gateway-db-backup-launchd.sh [options]

Install or reload a macOS launchd job that runs recurring gateway DB backups.
...
Options:
  ...
  --backup-dir PATH       Directory for local compressed backups
  --db-path PATH          Source SQLite database path override
  ...
  --label LABEL           launchd label (default: com.nexus.gateway-db-backup)
  --launchd-root PATH     Root-owned launchd asset root (default: /ai-data/launchd/nexus-gateway-db-backup)
EOF
}
...
LAUNCHER_DST="${BIN_DIR}/gateway-db-backup-launch-agent.sh"
JOB_ENV_FILE="${ENV_DIR}/gateway-db-backup.env"
...
sudo install -o root -g wheel -m 755 "$ROOT_DIR/deploy/scripts/gateway-db-backup-launch-agent.sh" "$LAUNCHER_DST"
...
if [[ -n "$DB_PATH" ]]; then
  append_env_line "DB_PATH" "$DB_PATH"
fi
...
<plist version="1.0">
  <dict>
    ...
    <array>
      ...
      <string>-lc</string>
      <string>exec /usr/bin/sudo -H -u ${TARGET_USER} env PATH=/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin HOME=${TARGET_HOME} NEXUS_GATEWAY_DB_BACKUP_ENV_FILE=${PLIST_JOB_ENV_FILE} /bin/bash ${PLIST_LAUNCHER_DST}</string>
    </array>
    <key>RunAtLoad</key>
    <true/>
